...
| P6 | Screenshot + Trace khi fail | ✅ Code xong — dry run + run thật; mở trace bằng `npx playwright show-trace` |
...
- [x] Code xong (`goto` default `load`; tùy chọn `domcontentloaded` trên từng bước)
- [x] Probe so baseline (AUTH3 5/5 STABLE_PASS)
- [x] User xác nhận sang P3
...
- [x] Code xong (unique locator; text click = button + link; dry run warn / run fail)
- [x] Probe / dry run ổn (user xác nhận)
- [x] User xác nhận sang P4
...
- [x] Code xong (2 retry cho lỗi transient; assert sai không retry)
...
- Screenshot khi fail (đã có từ trước) + **Playwright trace** (`failure.trace.zip`) khi fail
- Dry run + run thật dùng chung engine (`singleCaseExecutor`)
- API tải trace: `GET .../failure-trace` (quyền giống screenshot)
- Mở trace: `npx playwright show-trace failure.trace.zip`
- Chỉ lưu trace khi **fail**; pass thì discard tracing

- [x] Code xong
- [ ] Probe / dry run fail thử tải trace
- [ ] User xác nhận sang P7
...
| 2026-06-22 | P2 | Goto default `load`; AUTH3 probe STABLE_PASS |
| 2026-06-23 | P3 | Strict locator; modal ? Hướng dẫn selector; user xác nhận xong |
| 2026-06-23 | P4 | Step retry (2 lần) cho lỗi transient; assert mismatch không retry |
| 2026-06-29 | P6 | Failure Playwright trace (.zip) on fail; download API + UI |
