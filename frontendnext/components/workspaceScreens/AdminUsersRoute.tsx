...
export default function AdminUsersRoute() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const response = await apiRequest<{ users: RecordAny[] }>("/api/users", token);
        if (cancelled) return;
  ...
  const refreshUsers = async () => { const response = await apiRequest<{ users: RecordAny[] }>("/api/users", token); setUsers(Array.isArray(response.users) ? response.users : []); };
  ...
  const startUserEdit = (user: RecordAny) => { setEditingUserId(getId(user)); setNewUserForm({ name: user.name || "", email: user.email || "", password: "", role: user.role === "admin" ? "admin" : "employee" }); };
  const cancelUserEdit = () => { setEditingUserId(""); setNewUserForm({ name: "", email: "", password: "", role: "employee" }); };
