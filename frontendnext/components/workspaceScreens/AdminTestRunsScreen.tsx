  ...
  return (
    <div className="workspace-stack">
      ...
      <SectionCard title="Test Run List" subtitle="Start / completed runs">
        <DataTable columns={["Run", "Plan", "Progress", "Started by", "Status", "Action"]} rows={adminRuns.filter((run: RecordAny) => matchesSearch(run.name, run.testPlan?.name, userName(run.startedBy), run.status, run.progress)).map((run: RecordAny) => <><div>{run.name}</div><div>{run.testPlan?.name || "-"}</div><div>{typeof run.progress === "number" ? `${run.progress.toFixed(1)}%` : "0%"}</div><div>{userName(run.startedBy)}</div><div className={run.status === "running" ? "workspace-pill workspace-pill--success" : "workspace-pill"}>{run.status}</div><div><button type="button" className="workspace-secondary" onClick={async () => { setSelectedRunId(run._id); await loadMyItems(run._id); setActiveTab("execution"); }}>{run.status === "running" && String(run.startedBy?._id || run.startedBy || "") === currentUserId ? "Open" : "View"}</button></div></>)} emptyText="No runs" />
      </SectionCard>
    </div>
  );
}