...
export default function EmployeeMyTestPlansScreen({ scopedPlans, matchesSearch, setRunForm, setActiveTab }: Props) {
  return <div className="workspace-stack"><SectionCard title="My Test Plans" subtitle="Click vao test plan de vao run test"><DataTable columns={["Test Plan","Project","Version","Status","Progress","Action"]} rows={scopedPlans.filter((plan: RecordAny) => matchesSearch(plan.name, plan.project?.name, plan.version?.name, plan.status)).map((plan: RecordAny) => <><div>{plan.name}</div><div>{plan.project?.name || "-"}</div><div>{plan.version?.name || "-"}</div><div>{plan.status}</div><div>{plan.progress?.toFixed?.(1) ?? plan.progress ?? 0}%</div><div><button type="button" className="workspace-primary" onClick={() => { setRunForm((prev: RecordAny) => ({ ...prev, testPlanId: plan._id })); setActiveTab("execution"); }}>Run</button></div></>)} emptyText="No assigned plans" /></SectionCard></div>;
}