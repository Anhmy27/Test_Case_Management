...
function getWorkspaceHome(role?: string) {
  return role === "employee" ? "/workspace/employee" : "/workspace/admin";
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await apiRequest<{ user: { role?: string } }>("/api/auth/me", token, { method: "GET" });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (resp?.token) {
        ...
        router.replace(getWorkspaceHome((resp.user as { role?: string } | undefined)?.role));
      } else {
